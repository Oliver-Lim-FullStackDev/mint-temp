...
export class HeroGamesTransformer {
  ...
  static buildSearchBucket(params: {
    ...
    tags: string[];
    text?: string;
    providers?: RawGameProviderValue[];
  ...
  static normaliseSearchProviders(providerFilters: RawGameProviderValue[]): RawGameProviderValue[] {
    return providerFilters.reduce<RawGameProviderValue[]>((acc, value) => {
      ...
      const heroValue = this.normaliseProvider(trimmed) as RawGameProviderValue;
