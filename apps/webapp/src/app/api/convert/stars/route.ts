...
export async function GET(req: NextRequest) {
  try {
    ...
    // Call the backend API to convert to stars
    const conversion = await mintApi.get(`/convert/stars?amount=${amount}&from=${from}`);
    ...
    return NextResponse.json({ error: 'Failed to convert to stars' }, { status: 500 });
  }
}
