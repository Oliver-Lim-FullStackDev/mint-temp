...
export class SlotGameEngine extends MintGameEngine<SlotGameConfig, SlotGameResult> {
   ...
   */
  constructor(
    context: MintGameContext<SlotGameConfig>,
    dimensions: { reels?: number; rows?: number } = {},
  ) {
    super(context);
        ...
        for (let len = this.width - start; len >= 3; len--) {
          ...
          if (slice.every((s) => s === slice[0])) {
            ...
            if (this.config.payouts.enableMatchMultipliers) {
              ...
              const base = Number(this.config.payouts.rewards[symbol]?.[this.config.currency || 'SPN'] ?? 0);
    ...
    for (let start = 0; start <= this.width - 3; start++) {
      for (let len = this.width - start; len >= 3; len--) {
        const diag = Array.from({ length: len }, (_, i) => {
          if (start + i >= this.width || i >= this.height) {
            return null;
          }
          const column = grid[start + i];
          if (!column) {
            return null;
          }
          return column[i] ?? null;
  ...
  private checkSymbolCounts(grid: SlotGrid, res: WinningCombination[]): number {
    ...
    for (const [symbol, byCount] of Object.entries(this.config.payouts.symbolCounts)) {
      for (const [thresholdStr, payout] of Object.entries(byCount ?? {})) {
        const threshold = Number(thresholdStr);
        const actualCount = counts[symbol] ?? 0;
        if (actualCount >= threshold) {
          const reward = Number(payout ?? 0);
          const baseRewards = this.config.payouts.rewards[symbol]?.[this.config.currency] ?? 0;
          const totalReward = reward || baseRewards;
  ...
  private checkJackpot(grid: SlotGrid): number {
    const jackpotSymbol = this.config.payouts.jackpot.symbol;
    const jackpotCount = this.config.payouts.jackpot.count;

    if (!jackpotSymbol || jackpotCount <= 0) {
      return 0;
    }

    const actualCount = grid
      .flat()
      .filter((symbol) => symbol === jackpotSymbol).length;

    if (actualCount >= jackpotCount) {
      return this.config.payouts.jackpot.payout;
    }
